...
async function fetchTrips(url, key) {
  if (!key) return [];
  try { return extractTripUpdates(await fetchBuf(url, { headers: { Authorization: key } })); }
...
export async function arrivals(env, station, now) {
  ...
  const [metro, lr] = await Promise.all([
    fetchTrips(METRO_URL, env.BALTIMORE_METRO_KEY),
    fetchTrips(LR_URL, env.BALTIMORE_LR_KEY)
  ]);
