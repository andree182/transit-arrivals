...
- **Stations are findable by any of their platform names.** A merged complex keeps one display name, which made its other platforms unsearchable — "6 Av" (the L at 14 St), "Bleecker St", "Court Sq", "Port Authority" found nothing. Member platform names now ride along as search aliases and show on the result row ("14 St · 6 Av"), which also finally tells the three 14 Sts apart at a glance. Spelling is forgiving too: "6av", "6 ave", "6th Avenue", and "Sixth Ave" all land on the same station.
- **PATH stops reading like line noise.** A subway station with PATH service used to list the raw service codes among its bullets — "14 St (123FMLJHW3H3JS)". PATH now collapses to one word: "14 St (123FML · PATH)", on the result rows, in saved favorites, and on the watch's station footer.
