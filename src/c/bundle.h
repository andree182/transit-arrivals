...
typedef struct { char dest[21]; char dirLabel[11]; uint8_t n; uint16_t delta[MAX_ARR]; uint8_t expMask; } DirView;  // dirLabel: uppercase direction word, "" when none (v6); expMask bit a => arrival a is express (v5+)
